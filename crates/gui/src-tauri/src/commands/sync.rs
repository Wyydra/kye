...
pub fn generate_pairing_qr(port: u16, name: String, pin: String) -> AppResult<String> {
    let ip = get_local_ip().ok_or_else(|| AppError::Internal("Could not resolve local network IP address".into()))?;
    let url = format!("kye-remote://{}:{}?name={}&pin={}", ip, port, name, pin);
    let svg = infra::sync::generate_qr_svg(&url)
...
pub fn start_p2p_server(
    ...
    let service = state.service().ok_or_else(|| AppError::Internal("No active workspace to sync".into()))?;
...
pub async fn ping_remote_peer(remote_url: String) -> AppResult<HandshakeResponse> {
    tauri::async_runtime::spawn_blocking(move || {
        infra::sync::ping_remote(&remote_url)
            .map_err(|e| AppError::Internal(e))
    })
    .await
...
pub async fn push_to_remote_peer(remote_url: String, cmds: Vec<CommandDto>) -> AppResult<()> {
    ...
    tauri::async_runtime::spawn_blocking(move || {
        infra::sync::push_to_remote(&remote_url, domain_cmds)
            .map_err(|e| AppError::Internal(e))
    })
    .await
    .map_err(|e| AppError::Internal(format!("Runtime error: {:?}", e)))?
...
pub async fn pull_remote_peer_graph(remote_url: String) -> AppResult<GraphDto> {
    tauri::async_runtime::spawn_blocking(move || {
        infra::sync::pull_graph_from_remote(&remote_url)
            .map_err(|e| AppError::Internal(e))
    })
    .await
    .map_err(|e| AppError::Internal(format!("Runtime error: {:?}", e)))?
...
#[tauri::command]
pub fn get_local_tombstones(state: State<'_, AppState>) -> AppResult<std::collections::HashMap<String, String>> {
    let service = state.service().ok_or_else(|| AppError::Internal("No workspace selected".into()))?;
    let tombstones = service.load_tombstones()
...
#[tauri::command]
pub async fn pull_remote_peer_tombstones(remote_url: String) -> AppResult<std::collections::HashMap<String, String>> {
    tauri::async_runtime::spawn_blocking(move || {
        infra::sync::pull_tombstones_from_remote(&remote_url)
            .map_err(|e| AppError::Internal(e))
    })
    .await
    .map_err(|e| AppError::Internal(format!("Runtime error: {:?}", e)))?
