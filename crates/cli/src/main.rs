...
async fn main() -> anyhow::Result<()> {
    ...
    let service = Service::new(repo.clone(), repo, ()); // Le Unit type `()` implémente EventDispatcher no-op
    ...
    match cli.command {
        Commands::Add { content, type_name } => {
            ...
            let mut fields = metadata_provider.get_fields().unwrap_or_else(|e| {
        ...
        Commands::List => {
            let workspace: domain::models::workspace::Workspace = service.get_workspace().await?;
            println!("Blocks in workspace '{}' ({}):", workspace.name(), workspace.id());
            for block in workspace.blocks() {
                let content_preview = block.fields().get(&domain::models::block::schema::FieldName::new("body")).and_then(|v| v.as_str()).unwrap_or_default().to_string();
        ...
        Commands::Show { id } => {
            let workspace: domain::models::workspace::Workspace = service.get_workspace().await?;
            if let Some(block) = workspace.blocks().iter().find(|b: &&domain::models::block::Block| *b.id() == id) {
                println!("Block ID: {}", block.id());
                println!("Fields: {}", block.fields());
            } else {
