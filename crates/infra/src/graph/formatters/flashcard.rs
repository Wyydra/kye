...
impl BlockFormatter for FlashcardFormatter {
    fn kind(&self) -> &'static str { "core.flashcard" }

    fn matches(&self, text: &str) -> bool {
        let lines: Vec<&str> = text.lines().map(|l| l.trim()).collect();
        if lines.is_empty() { return false; }
        let first = lines[0];
        first.starts_with("> **Front**:") || first.starts_with("> **Front** :") || first.starts_with("> **Question**:")
    }

    fn native_keys(&self) -> &'static [&'static str] { &["front", "back"] }

    fn format(&self, props: &Props) -> String {
        let front = props.get(&PropKey::from("front"))
            .map(value_to_markdown)
            .unwrap_or_default();
        let back = props.get(&PropKey::from("back"))
            .map(value_to_markdown)
            .unwrap_or_default();
        format!("> **Front**: {}\n>\n> **Back**: {}", front, back)
    }
    ...
    fn extract(&self, text: &str) -> Props {
        ...
        for line in text.lines() {
            let clean = line.strip_prefix("> ").unwrap_or(line.strip_prefix(">").unwrap_or(line)).trim();
        ...
        props.insert(PropKey::from("front"), Value::Rich(markdown_to_rich(&front_text)));
        props.insert(PropKey::from("back"), Value::Rich(markdown_to_rich(&back_text)));
        props
    }
}
